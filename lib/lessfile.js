...
LessFile.prototype = {
  ...
  _addSourceMapProperties: function (compileOptions, self) {
    // only gets called if compileOptions.sourceMap = true
    compileOptions.writeSourceMap = function (sourceMapContent) {
      var orig = self.dstFile;
      self.dstFile = self.dstFile + ".map";
      fs.writeFileSync(self.dstFile, sourceMapContent, 'utf-8');
      self.dstFile = orig;
    };
    compileOptions.sourceMapURL = path.basename(self.dstFile) + ".map";
  },
  ...
  compile: function(compileOptions, callback) {
    ...
    this._getParser().parse(fs.readFileSync(this.file, 'utf-8'), function(err, tree) {
      if (err) {
        ...
        try {
          self._addSourceMapProperties(compileOptions, self);
